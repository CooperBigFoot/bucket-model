...
def preprocess_data(path_to_file: str, catchment_area: float, output_destination: str = '') -> None:
    ...
    - save (bool): If True, the preprocessed data will be saved to a new .csv file. If False, the function will only return the preprocessed data.
    """

    precipitation = pd.read_csv(path_to_file, sep='\s+', skiprows=1, header=0)

    precipitation = precipitation.set_index('day')
    ...
    # Only keep data from 1986  to 1999
...
def main() -> None:
    ...
    preprocess_data(path_to_file, output_destination, catchment_area)
